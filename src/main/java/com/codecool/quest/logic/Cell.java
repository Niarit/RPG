...
public class Cell implements Drawable {
    ...
    public Cell getNeighbor(int dx, int dy) {
        try {
            ...
            if (cellToMove.type == CellType.WALL || cellToMove.type == CellType.EMPTY || cellToMove.type == CellType.WINE || cellToMove.type == CellType.LEFT_CORNER|| cellToMove.type == CellType.RIGHT_CORNER|| cellToMove.type == CellType.RUINED_WALL_PIECE_LEFT || cellToMove.type == CellType.RUINED_WALL_PIECE_RiGHT|| cellToMove.type == CellType.ROCK || cellToMove.type == CellType.TREE) {
    ...
    private void combat(Cell cellToMove, Cell playerCell) {
        ...
        if (cellToMove.getActor().getHealth() <= 0) {
            if(cellToMove.getActor().getTileName().equals("zombie")){
                cellToMove.setActor(new Skeleton(cellToMove));
            }
